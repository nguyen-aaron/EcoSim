...
export default function Simulate() {
  ...
  // optional full history (new hook may not expose this)
  const historyRef = worker.historyRef;

  const cleanParams = (raw) => {
    const result = { ...raw };
    for (const f of config.fields) {
      const k = f.key;
      if (result[k] === "") result[k] = config.defaults[k];
    }
    return result;
  };
  ...
  const handleDownloadCSV = () => {
    // Prefer full history if provided by the hook; otherwise use current buffers
    const fullPrey = historyRef?.current?.prey ?? prey ?? [];
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              ...
              disabled={!paused || (prey.length === 0 && predator.length === 0)}
            >
              Download CSV
            </button>
          </div>
